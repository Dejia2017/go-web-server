...
func InitLogs() {
	consoleLogs = logs.NewLogger(1)            //缓冲大小
	consoleLogs.SetLogger(logs.AdapterConsole) //实现一个控制台日志取代系统默认的
	consoleLogs.Async()                        //异步 可以提升性能
	fileLogs = logs.NewLogger(10000)
	level := beego.AppConfig.String("logs::level")
	fileLogs.SetLogger(logs.AdapterMultiFile, `{"filename":"logs/gws.log",
		"separate":["emergency", "alert", "critical", "error", "warning", "notice", "info", "debug"],
		"level":`+ level+ `,
		"daily":true, 
		"maxdays":10}`)
	//daily 按照每天 logrotate 默认true level=7 debug
	fileLogs.Async() //异步
